...
- Phase 1 ships **pure static HTML/CSS**, with **no HTMX and no JS build**. HTMX arrives in
  phase 2 with the booking/ordering endpoints (see D3).
...
- **No dynamic endpoints, no HTMX, no JS in phase 1.** No booking/ordering handlers, forms,
  DB, or auth **yet**; the architecture must accommodate them (D6).
...
- **No production runtime provisioning** (Cloudflare/EC2) this iteration; that is phase 2.
...
- **D4. Styling: hand-written CSS, single stylesheet** (`assets/css/site.css`, copied to
  ...
  fonts) centralised so a rebrand is a one-file change. Tailwind rejected (needs Node, a non-goal).
...
- **D7. AI-crawler allowlist via `robots.txt`.** `static/robots.txt` (copied to the site
  ...
  Google's index, and allowing `Googlebot` also restores normal Google Search), DeepSeek
  (`DeepSeekBot`) and Moonshot/Kimi (`Kimi-User`). The DeepSeek and Kimi tokens are from the
  community `ai-robots-txt` list, not first-party docs, and are marked unverified in the file.
...
- **R1. Pages sub-path breaks absolute asset/nav URLs.** Project Pages serve under `/<repo>/`.
  ...
  asset/nav URL, empty locally and `/hino-hilux` in CI. Verify the deployed URL, not just `dist/`.
...
- **S1. Walking skeleton to Pages, server-first.** `views/base.templ` + one page, the route
  ...
  `go run ./cmd/server` serves the same locally. (Proves the pipeline + dual-runtime seam, the
  highest-risk parts, first.)
